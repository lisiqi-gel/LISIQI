...
import checkemail from '../Service/CheckemailApi';
...
const handleRegister = async () => {
  try {
    const res = await RegisterApi(email,company,companyweb,password);
    Modal.success({
      title: '登録完了',
      content: '登録が正常に完了しました。ログインページに移動します。',
      onOk: () => navigate('/'),
    });
  } catch (error: any) {
    Modal.error({
    title: '登録失敗',
    content: error.message || '登録中にエラーが発生しました',
  });
}
}
  // 验证邮箱 格式正确 
      ...
      <div className={styles.formContainer}>
        ...
        <div className={styles.section}>
          ...
          <Input
            className={styles.input}
            value={email}
            placeholder="○○○○＠会社ドメイン.jp/.com"
            prefix={<MailOutlined />}
            onChange={(e) => setEmail(e.target.value)}
            onBlur={async (e) => {
              const value = e.target.value;
              const formatError = validateEmail(value, companyweb);
              ...
              if (formatError === '') {
                try {
                  ...
                  setEmailError('メール確認中にエラーが発生しました');
                }
              }
            }}
          />
        ...
        <div className={styles.section}>
          <p className={styles.label}>
            ...
            <>
            <br /><span className={styles.error}>
              入力したパスワードが一致しません!
            </span></>)}
            </p>
          <Input.Password
